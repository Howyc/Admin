...
const { isLeft, isTop, isLeftTop } = useLayoutMode() // 确保正确解构赋值
const settingsStore = useSettingsStore()
const { showSettings, showTagsView, showWatermark, showGreyMode, showColorWeakness } = storeToRefs(settingsStore)

const classes = computed(() => {
  const classesArray: string[] = []
  if (showGreyMode.value) classesArray.push("showGreyMode")
  if (showColorWeakness.value) classesArray.push("showColorWeakness")
  return classesArray
})
